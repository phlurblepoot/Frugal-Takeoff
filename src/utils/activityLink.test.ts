...
describe('activityTarget', () => {
  it('routes each vertical to its section', () => {
    expect(activityTarget({ type: 'invoice_created', projectId: 'p1' })).toBe('/project/p1/billing');
    expect(activityTarget({ type: 'payment_recorded', projectId: 'p1' })).toBe('/project/p1/billing');
    expect(activityTarget({ type: 'change_order_sent', projectId: 'p1' })).toBe('/project/p1/billing');
    ...
    expect(activityTarget({ type: 'proposal_sent', projectId: 'p1' })).toBe('/project/p1/proposal');
  });
