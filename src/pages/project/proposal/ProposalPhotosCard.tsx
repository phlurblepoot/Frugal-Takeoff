...
export const ProposalPhotosCard: React.FC<{
  ...
  const handlePick = async (rows: DocumentRow[]) => {
    for (const row of rows) {
      try { await addProposalPhoto(proposal.id, row.id); } catch { /* keep going */ }
    }
    onChanged();
  };
  ...
  const handleCaption = async (photo: ProposalPhoto, caption: string) => {
    try {
      ...
      onChanged();
    } catch { toast('Failed to update caption', { type: 'error' }); }
  };
  ...
  const handleMove = async (index: number, dir: -1 | 1) => {
    ...
    try {
      await Promise.all([
        updateProposalPhoto(cur.id, cur.fileId, { sortOrder: other.sortOrder }),
        updateProposalPhoto(other.id, other.fileId, { sortOrder: cur.sortOrder }),
      ]);
      onChanged();
    } catch { toast('Failed to reorder photos', { type: 'error' }); }
  };
  ...
  const handleRemove = async (photo: ProposalPhoto) => {
    try {
      ...
      onChanged();
    } catch { toast('Failed to remove photo', { type: 'error' }); }
  };
