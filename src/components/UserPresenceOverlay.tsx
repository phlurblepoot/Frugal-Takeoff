...
import { Users, X, ExternalLink, UserCheck } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <button
        ...
        className="w-12 h-12 bg-white rounded-full shadow-lg border border-slate-200 flex items-center justify-center text-slate-600 hover:text-blue-600 hover:border-blue-200 transition-all active:scale-95 relative"
      >
        <Users size={24} />
        {otherUsers.length > 0 && (
            ...
            <div className="max-h-96 overflow-y-auto p-2">
              {otherUsers.length === 0 ? (
                <div className="p-8 text-center">
                  <p className="text-sm text-slate-500 italic">No other users online</p>
                </div>
              ) : (
                <div className="space-y-1">
                  {otherUsers.map(user => (
                    ...
                    >
                      ...
                      <label className="flex items-center gap-1.5 cursor-pointer p-1.5 rounded-lg hover:bg-white border border-transparent hover:border-slate-200 transition-all">
                        <input
                          type="checkbox"
                        ...
                        />
                        <span className={`text-[10px] font-bold uppercase tracking-wider ${followedUserId === user.id ? 'text-blue-600' : 'text-slate-400'}`}>
