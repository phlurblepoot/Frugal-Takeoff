...
- `src/components/documents/AddFilesButton.tsx`: `{ label, accept, multi, upload, initialProjectIds, excludeFileIds, onPick | onPickBlobs }` — one button, opens the modal on the Upload tab when nothing exists yet? No — always opens on **Existing** unless `defaultTab='upload'`; photo cards pass `defaultTab='upload'` (uploading new photos is the common case), EmailComposer/attachments pass `'existing'`.
